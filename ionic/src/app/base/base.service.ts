...
export class BaseService {
    ...
    constructor(injector: Injector, factory: Serializable = null) {
    ...
    get(
        ...
        return this.http
            .get(
                this.buildUrl('/get', relations, wheres, paginate),
                this.api_token
            )
            .toPromise();
    }
    ...
    find(relations: Array<any> = [], wheres: Object = {}): Promise<any> {
        return this.http
            .get(this.buildUrl('/find', relations, wheres), this.api_token)
            .toPromise();
    }
    ...
    store(model: Serializable): Promise<any> {
        return this.http
            .post(this.buildUrl(), model.http_data, this.api_token)
            .toPromise();
    }

    update(model: Serializable): Promise<any> {
        return this.http
            .put(this.buildUrl('/' + model.id), model.http_data, this.api_token)
            .toPromise();
    }

    buildPromise(data: any): Promise<any> {
        return this['$' + data.method](data);
    }

    $put(data: any): Promise<any> {
        return this.http
            .put(data.url, data.data ? data.data : {}, this.api_token)
            .toPromise();
    }

    $post(data: any): Promise<any> {
        return this.http
            .post(data.url, data.data ? data.data : {}, this.api_token)
            .toPromise();
    }
    ...
    destroy(id: any): Promise<any> {
        return this.http
            .delete(this.buildUrl('/' + id), this.api_token)
            .toPromise();
    }

    restore(id: any): Promise<any> {
        return this.http
            .put(this.buildUrl('/' + id + '/restore'), this.api_token)
            .toPromise();
    }
    ...
    $paginations(paginate: any) {
        return (
            '?sorting[' +
            paginate.sorting.column +
            ']=' +
            paginate.sorting.value +
            '&count=' +
            paginate.take +
            '&page=' +
            paginate.page +
    ...
    $relations(relations: Array<string>) {
        ...
        if (relations.length) {
            relations.forEach((relation) => {
                url_string +=
                    'relations[]=' + encodeURIComponent(relation) + '&';
            });
        }
    ...
    $wheres(wheres: Object) {
        ...
        keys.forEach((key) => {
            if (Array.isArray(wheres[key])) {
                wheres[key].forEach((value, index) => {
