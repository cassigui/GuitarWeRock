...
export class ResetPasswordPage implements OnInit {
    ...
    sendEmail() {
        this.helperService.loading('Sending e-mail');
        this.userService.sendForgotPasswordEmail(this.data).then(
            (response) => {
    ...
    validateResetToken() {
        this.helperService.loading('Validating');
        this.userService.validateResetToken(this.data).then(
            (response) => {
    ...
    updatePassword() {
        this.helperService.loading('Updating');
        this.userService
            .updatePassword(this.data, ['access_level.permissions'])
            .then(
                (response) => {
                    this.helperService.loading_dismiss();
                    if (response.error) {
